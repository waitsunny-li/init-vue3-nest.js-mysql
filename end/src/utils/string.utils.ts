...
export class StringUtils {
  ...
  public static indexOf(str: string, searchStr: string, startPos?: number): number {
    if (ObjectUtils.isNullOrUndefined(str) || ObjectUtils.isNullOrUndefined(searchStr) || (!ObjectUtils.isUndefinend(startPos) && !NumberUtils.isSafeInteger(startPos))) {
  ...
  public static lastIndexOf(str: string, searchStr: string, position?: number): number {
    if (ObjectUtils.isNullOrUndefined(str) || ObjectUtils.isNullOrUndefined(searchStr) || (!ObjectUtils.isUndefinend(position) && !NumberUtils.isSafeInteger(position))) {
      return this.INDEX_NOT_FOUND
    }
  ...
  public static subString(str: string, start: number, end?: number): string {
    if (!ObjectUtils.isString(str) || !NumberUtils.isSafeInteger(start) || (!ObjectUtils.isUndefinend(end) && !NumberUtils.isSafeInteger(end))) {
      return null
    }
  ...
  public static isTimeStr(str: string): boolean {
    return (
      str.search(
        /(([0-9]{3}[1-9]|[0-9]{2}[1-9][0-9]{1}|[0-9]{1}[1-9][0-9]{2}|[1-9][0-9]{3})-(((0[13578]|1[02])-(0[1-9]|[12][0-9]|3[01]))|((0[469]|11)-(0[1-9]|[12][0-9]|30))|(02-(0[1-9]|[1][0-9]|2[0-8]))))|((([0-9]{2})(0[48]|[2468][048]|[13579][26])|((0[48]|[2468][048]|[3579][26])00))-02-29)/i,
      ) > -1
    )
  }
   /**
   * 判断12:12:44 或者 12:33等时间的
   * @example StringUtils.delInString('1,2,3', '2')    = '1,3';
   * @example StringUtils.delInString("null, 2, 6", 2)    = '6';
   */
  public static isTime(time: string): boolean {
    if (Object.prototype.toString.call(time) !== '[object String]') return;
    let timeList = time.split(':')
    if (timeList.length < 2) return;
    console.log(Number(timeList[0]),);

    let isAllNumber = timeList.every((item, index) => {
      if (Number.isNaN(Number(item))) {
        return false
      }
      if (index === 0) {
        if (0 <= Number(item) && Number(item) <= 24) return true;
        return false
      }
      if (0 <= Number(item) && Number(item) <= 60) return true;
      return false
    })
    return isAllNumber
  }
   ...
   * @example StringUtils.delInString("null, 2, 6", 2)    = '6';
   */
  public static delInString(originalString: string, s: string | number): string {
    let orArray = originalString.split(',') || []
    let newArray = orArray.filter(item => item != 'null' && item != 'undefined' && item != s)
